"""
E2E Fase 3 — VISUAL_ANALYSIS contra vault real.
...
def fetch_pending_transcribe_tasks(conn: sqlite3.Connection, obra: str):
    cur = conn.execute(
        ...
        FROM tasks
        WHERE task_type='visual_analysis' AND status='pending' AND obra=?
...
def run(obra: str, throttle_sec: float = 0.3) -> RunStats:
    ...
    console.print(f"[bold cyan]E2E Fase 3 — VISUAL_ANALYSIS[/bold cyan]")
    ...
    with Progress(
        ...
        bar = progress.add_task(
            "[green]Analisando imagens...", total=stats.total,
        ...
        for row in rows:
            ...
            try:
                t0 = time.monotonic()
                result_ref = visual_analysis_handler(task, conn)
    ...
    sentinel_row = conn.execute(
        """SELECT COUNT(*) FROM visual_analyses va
           JOIN files f ON f.file_id = va.file_id
           WHERE va.obra=? AND va.confidence=0.0""",
        (obra,),
    ).fetchone()
...
def print_summary(stats: RunStats) -> None:
    table = Table(title="E2E Fase 3 — VISUAL_ANALYSIS", show_header=False, show_lines=False)
    table.add_column("métrica", style="cyan", no_wrap=True)
    table.add_column("valor", style="bold")

    table.add_row("total", str(stats.total))
    table.add_row("done", f"[green]{stats.done}[/green]")
    table.add_row("sentinel (JSON inválido)", f"[yellow]{stats.sentinel}[/yellow]")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="E2E Fase 3 — VISUAL_ANALYSIS")
