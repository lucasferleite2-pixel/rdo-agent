...
>
> **Última atualização:** 25/04/2026 — versão rdo-agent v1.6-scale-analytics (Sessão 10: correlator paralelo + janela by-detector + narrator hierárquico + cache binário; ADRs 010/011/012; dívidas #61/#62/#63 registradas). **GRUPO 4 (escala analítica) parcial — Sessão 11 valida em corpus grande.**
...
✅ v1.6    — Escala analítica (Sessão 10 — #50, #51, #52):
              correlator paralelo (4 workers ProcessPool) com
              janela by-detector configurável; narrator hierárquico
              cascade day→week→month→overview com migration relax
              CHECK + file_ids preservation; cache binário de
              narrativas (prompt_template_hash + dossier_hash).
              ADRs 010/011/012 trazem rationale e triggers para
              dívidas adiadas. 51 testes novos. 899 testes total
              verde. Custo $0 (validação não-destrutiva).
              **GRUPO 4 parcial — Sessão 11 valida em corpus grande.**
...
### 9.12 Resolvidas em Sessão 10 (v1.6-scale-analytics)

3 dívidas fechadas + 3 novas registradas:

- ~~#50~~: correlator paralelo + janela by-detector — `b1c94c0`.
  ProcessPoolExecutor 4 workers (cada com conn própria SQLite),
  persistência centralizada no main process, `DetectorWindows`
  com helper `all_days(N)`. Cada detector ganhou kwarg
  `window: timedelta | None`. ADR-010.
- ~~#51~~: narrator hierárquico cascade day→week→month→overview —
  `73dedc0`. Migration `_migrate_sessao10_relax_narratives_scope_check`
  remove CHECK constraint legado em scope; `narrator.VALID_SCOPES`
  vira single source of truth. `compose_input_from_children`
  preserva file_ids (rastreabilidade forense). ADR-011.
- ~~#52~~: cache binário de narrativas — `87b7e03`. Migration
  adiciona `prompt_template_hash`. `NarrativeCacheManager.get(...)`
  hit em 5 dimensões; `annotate_hash` hidrata legacy; `invalidate`
  cirúrgico. **Sem fuzzy** — adiado em #62. ADR-012.

**Dívidas novas registradas (não fechadas — adiadas com triggers)**:

- **#61** — Paralelismo intra-detector. Triggers: corpus 100k+
  com detector individual dominando tempo total, profiling
  identifica bottleneck, Sessão 11 reporta gargalo. Estimativa:
  1 sprint pequena por detector identificado.
- **#62** — Cache fuzzy de narrativas (similarity > 0.95 = hit).
  Triggers: typo-induced re-pagamento reportado, 3+ ocorrências,
  custo agregado > $5. Estimativa: meio dia. Pode usar Jaccard
  inicial ou esperar #59.
...
### 9.13 Pendentes (pós-v1.6) — 4 abertas + #59 + #60 + #61 + #62 + #63
...
| #61 | Paralelismo intra-detector (sob trigger) | quando trigger ativar |
| #62 | Cache fuzzy de narrativas (sob trigger) | quando trigger ativar |
| #63 | narrate/correlate como task_types (sob trigger) | quando trigger ativar |

> **Total fechadas:** 46 (anteriores + #50, #51, #52 desta sprint).
> **Total abertas:** 4 + 5 dívidas adiadas (#59, #60, #61, #62, #63)
> com triggers explícitos.
> **ADRs ativos:** ADR-007 (state machine wrapper); ADR-008 (classify
> 3-tier); ADR-009 (vision cascade); ADR-010 (correlator paralelo);
> ADR-011 (narrator hierárquico); ADR-012 (cache binário narrativas).
...
## 11. Métricas Atuais (v1.6, verificadas 25/04/2026)
...
Código:
Commits totais:      ~115+
Tags publicadas:     19 versões + 15 safety checkpoints
Testes passando:     899 (após Sessão 10 — scale-analytics +51 novos)
Arquivos Python:     ~80+
Linhas de código:    ~14.500+

Custos acumulados até v1.6:
...
Sessão 10 (scale analytics):   US$ 0.00 (read-only + fixtures)
Total:               ~US$ 3.16 (≈ R$ 16)
```
